...
class ColorTracker:
    ...
    def _process_frame(self) -> None:
        ...
        if self._processing_type == ProcessingType.TRACKER:
            ...
            hsv_frame: np.ndarray = cv2.cvtColor(self._frame, cv2.COLOR_BGR2HSV)
    ...
    def _create_tracker_for_object(self, frame: None | np.ndarray) -> None | np.ndarray:
        """
        ...
        for idx, color in enumerate(self._tracked_colors):
            ...
            for contour in contours:
                if cv2.contourArea(contour) > 100:
                    ...
                    cv2.putText(frame, f"Object_{idx + 1}", (x, y - 10),
